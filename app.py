...
def view_activity():
    ...
    with st.sidebar:
        ...
        chapter_name = st.sidebar.selectbox("メニューを選択してください",(chapters))
        ...
        lesson_name = st.sidebar.selectbox("レッスンを選択してください",(lessons))
    ...
    if st.session_state['activity_steps'] >= 1:
        ...
        with st.sidebar:
            ...
            # 演習ステップ
            step_name = st.radio("演習を選択してください", step_list, horizontal=True, on_change = init_activity, args=(chapter_name, lesson_name, activity_name,))
...
def main():
    ...
    # ポータル画面の表示
    if st.session_state['portal_flg'] == False:
        view_portal()

    # アクティビティ画面の表示
    if st.session_state['portal_flg'] == True and st.session_state['login_flg'] == True:
        view_activity()
